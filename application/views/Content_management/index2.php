							...
							<div class="form-group">
								...
								<div class="custom-file">
									<input type="file" class="custom-file-input" id="validatedCustomFile" required
										name="campaign_image" onchange="previewFile(this, 1);">
...
<script>
	...
	$('#saveButton').click(function () {
		var formData = {
			...
			email_template: $("#summernote_email").summernote('code'),
			landingpage_template: $("#summernote_landingpage").summernote('code'),
		};
		$.ajax({
			type: 'POST',
	...
	var filterCount = 0;
	var filterField = [{"id":"kec_speedy","text":"Kecepatan Internet"},{"id":"payment","text":"Metode Pembayaran"}];
	...
	$('#addFilter').click(function () {
		...
		$('#field'+filterCount).select2({data:filterField});
	...
	function getFieldData(id) {
		let selectedField = document.getElementById("smsTextArea").value
		var formData = {
			sms_template: document.getElementById("smsTextArea").value,
			email_template: $("#summernote_email").summernote('code'),
			landingpage_template: $("#summernote_landingpage").summernote('code'),
		...
		$.ajax({
			...
			dataType: 'json',
			success: function (result) {
				$('#filter'+id).setAttribute('multiple','multiple');
				$('#filter'+id).select2({data:result});
			},
			error: function (data) {
				console.log(data);
			}
